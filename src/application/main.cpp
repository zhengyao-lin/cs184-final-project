...
int main( int argc, char** argv ) {
  ...
  while ( (opt = getopt(argc, argv, "s:l:t:m:e:h:H:f:r:c:b:d:a:p:L:R:S:")) != -1 ) {  // for each option...
    switch ( opt ) {
    case 'f':
      write_to_file = true;
    ...
    case 'H':
      ...
      break;
    case 'R':
      config.lens_flare_max_reflection = atoi(optarg);
      break;
    case 'S':
      config.lens_flare_samples = atoi(optarg);
      break;
    default:
      usage(argv[0]);
      return 1;
